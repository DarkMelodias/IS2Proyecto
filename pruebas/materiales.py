...
class Materiales:
    ...
    def consulta_materialesN(self):
        cur = self.cnn.cursor()
        cur.execute("SELECT nom_material FROM materiales")
        datos = cur.fetchall()
        if len(datos) == 0:
            cur.close()
            return None
    ...
    def buscar_materialesN(self, num):
        cur = self.cnn.cursor()
        sql = "SELECT * FROM materiales WHERE nom_material='{}' ".format(num)
        cur.execute(sql)
        datos = cur.fetchone()
        if datos == None:
            cur.close()
